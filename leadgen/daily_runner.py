#!/usr/bin/env python3
"""
Daily Lead Generation Runner — All free sources, no API keys required.

Sources (in priority order):
  1. OpenStreetMap Overpass API — free, no key, returns phone+website
  2. Yellow Pages — free HTML scraping
  3. State Secretary of State filings — free, finds brand new businesses

Usage:
  python leadgen/daily_runner.py                    # Full daily run
  python leadgen/daily_runner.py --state MA         # Single state
  python leadgen/daily_runner.py --source overpass  # OSM only
...
from utils import enrich_lead, calculate_lead_score
...
def run_overpass(states=None, max_per_city=30, only_no_website=False):
    ...
    for state in states:
        ...
        leads, error = scrape_overpass(state, max_per_city=max_per_city, only_no_website=only_no_website)
        ...
        log_scrape(f"Overpass-{state}", state, len(leads), added, "completed", error[:500] if error else "")
...
def run_yellowpages(states=None, max_per_combo=10, only_no_website=False):
    ...
    for state in states:
        ...
        leads, error = scrape_yellowpages(state, max_per_combo=max_per_combo, only_no_website=only_no_website)
        ...
        log_scrape(f"YP-{state}", state, len(leads), added, "completed", error[:500] if error else "")
...
def run_sos_scrapers(states=None, days_back=7, max_per_state=100):
    ...
    for state in states:
        ...
        if leads:
            ...
            log_scrape(f"SOS-{state}", state, len(leads), added, "completed", error[:500] if error else "")
        else:
...
def main():
    ...
    parser.add_argument("--source", choices=["overpass", "yp", "sos", "all"], default="all", help="Data source")
    ...
    parser.add_argument("--only-no-website", action="store_true", help="Only return businesses without websites")
    parser.add_argument("--report-only", action="store_true", help="Only print report")
    args = parser.parse_args()

    init_db()

    print(f"One Vision Marketing — Lead Generator (FREE sources)")
